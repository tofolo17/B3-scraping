...
for i in range(len(empresas)):
    ...
    for j in range(2, len(items.options) + 1):
        if int(anos[j - 1]) >= 2010:
            driver.c(f'//*[@id="ctl00_contentPlaceHolderConteudo_cmbAno"]/option[{j}]', False)

            # Pega o Formulário de Referência
            try:
                elemento_formulario = WebDriverWait(driver, 10).until(ec.element_to_be_clickable(
                    (By.ID, "ctl00_contentPlaceHolderConteudo_rptDocumentosFRE_ctl00_lnkDocumento"))
                )
                formulario = elemento_formulario.get_attribute("href")
            except Exception as e:
                print(f"Sem formulário: {e}")
                formulario = None
            ...
            while True:
                try:
                    if formulario is not None:
                        ...
                        diretores_final = [diretores for diretores in diretores_bruto[3:]]

                        """
                        info_spec = []
                        # Informações específicas
                        qtd_diretores = floor(len(diretores_final) / 3)
                        ...
                        with open("data", "a", encoding="utf-8") as f:
                            ...
                            # Informações gerais
                            for k in range(0, len(diretores_final), 3):
                                print(k)
                                f.write(
                                    f'{empresas[i]};'  # Empresa
                                    f'{anos[j - 1]};'  # Ano
                                    f'{diretores_final[k]};'  # Nome
                                    f'{diretores_final[k + 1] if diretores_final[k + 1] != "" else "Não informado."};'
                                    #  CPF
                                    f'{diretores_final[k + 2]}\n'  # Cargo eletivo ocupado
                                    # f'{info_spec[int(k / 3)]}\n'
                                )
